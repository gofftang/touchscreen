...
unsigned int PointMOne(unsigned int x0,unsigned int x1)
{
	if(x1 >= x0)
		return 0;
	if(x1 < edge_start && x1*3/2 < x0)
		return 1;
	else if(x1 < edge_start*2 && x1*2 < x0)
		return 1;
	return 0;
}
...
void PointMenu(void)
{
	...
	if(point_delay[0].other.delay<1 || point_delay[0].other.report<1)
		return;
	if(PointDistance(&pp[0][0],&pp[1][1]) < (shake_min > 6*6 ? shake_min : 6*6))
		return;
